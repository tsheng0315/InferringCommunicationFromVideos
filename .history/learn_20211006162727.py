...
# read in arguments
# MODEL_FOLDER=results/team
# MODEL=TeamModel2

# model_name = sys.argv[1]  # $MODEL_FOLDER/fold-0 
# model_type = sys.argv[2]  # $MODEL
# data_fold = int(sys.argv[3])  0 
...
batch_size = 300
training_set = data.k_fold_training(data_fold, 1)
testing_set = data.k_fold_testing(data_fold, 1)
...
net = Model(2, 39 + 63 + 63+6, 4, dropout_p=0.2)
net.cuda()
...
for epoch in range(500):
    ...
    results_epoch = np.zeros((2, 4, 4),dtype=np.int32)
    ...
    for video_id in training_set:
        ...
        results = np.zeros((2, 4, 4),dtype=np.int32)
    ...
    net.eval()
    test_results = np.zeros((2, 4, 4),dtype=np.int32)
